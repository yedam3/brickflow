<template>
    <div class="card border-0" style="height: calc(100vh - 8rem);">
        ...
        <div class="d-flex flex-wrap justify-content-end gap-2 text-end mt-3 mb-4">
            <Button label="조회" severity="success" class="" @click="checkResultList" />
            <Button label="등록" severity="info" class="" @click="checkAdd" />
            <Button label="수정" severity="help" class="" @click="checkUpdate" />
            <Button label="삭제" severity="danger" class="" @click="checkDelete" />
        </div>
        <div class="row">
            <div class="col">
                <div class="heading-with-line mb-3">
                    <h5 class="m-0 me-3">자재 검수 목록</h5>
                </div>
                <!-- 검색창 -->
                ...
                <div class="ag-theme-alpine col me-5" style="height: 400px;">
                    <!-- ag-Grid 테이블 -->
                    <ag-grid-vue class="ag-theme-alpine custom-grid-theme" style="width: 100%;
                    height: 100%" :rowData="rowData" :columnDefs="columnDefs" :gridOptions="gridOptions"
                        @cellClicked="clicked" />
                </div>
            </div>
            <div class="col">
        ...
        error_check_ary: {
            handler(errAry) {
                let errTotal = errAry.reduce((errSum, errInfo, idx) => {
                    ...
                    //2) 총 불량량의 합계
                    return errSum += errInfo.mat_check_error;
                }, 0);
