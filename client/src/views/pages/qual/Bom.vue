<template>
  <div class="card border-0" style="min-height: calc(100vh - 8rem)">
    ...
    <div class="heading-with-line mb-2">
      <h5 class="m-0 me-3">BOM 관리</h5>
    </div>
    <div class="d-flex flex-wrap justify-content-end gap-2 text-end mb-3">
      <Button label="초기화" severity="help" class="" @click="resetList" />
      <Button label="저장" severity="info" class="" @click="bomSave" />
      <Button label="행추가" severity="success" class="" @click="addRow" />
    ...
    <div class="row">
      <div class="col">
        ...
        <ag-grid-vue style="width: 700px; height: 500px;" class="ag-theme-alpine" :columnDefs="columnDefs"
          :rowData="rowData" :gridOptions="gridOptions1" :defaultColDef="defaultColDef" @cellClicked="prodCellClicked">
        </ag-grid-vue>
      </div>
      <div class="col">
        <div class="heading-with-line mb-2">
          <h5 class="m-0 me-3">BOM 목록</h5>
        </div>
        <ag-grid-vue style="width: 700px; height: 500px;" ref="secondGrid" class="ag-theme-alpine"
          :columnDefs="columnDefs2" :rowData="rowData2" :gridOptions="gridOptions" rowSelection="multiple"
          :defaultColDef="defaultColDef2" @cellClicked="bomCellClicked">
        </ag-grid-vue>
      </div>
    </div>
  </div>
  <!-- 자재코드 모달창-->
  <MatModal :visible="showMatModal" rowSelection="multiple" @close="showMatModal = false" @selectMat="matSelected">
  </MatModal>
